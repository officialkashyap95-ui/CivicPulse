...
export default function LandingPage() {
  ...
  return <main className="site-shell">
    <header className="site-header"><a href="#top" className="brand"><span className="logo-mark"><Activity /></span><span>Civic<span>Pulse</span></span></a><nav className={menuOpen ? 'mobile-open' : ''}><a href="#how">How it works</a><a href="#impact">Impact scoring</a><a href="#platform">Platform</a><a href="#contact">For authorities</a></nav><div className="header-actions"><button className="sign-in">Sign in</button><button className="header-cta">Request a demo <ArrowRight data-icon="inline-end" /></button></div><button className="menu-button" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu">{menuOpen ? <X /> : <Menu />}</button></header>
    <section id="top" className="hero-section"><div className="hero-copy"><div className="announcement"><span><span className="status-dot" /> Built for cities that listen</span><ArrowRight data-icon="inline-end" /></div><h1>Report problems.<br /><span>We prioritize</span> what matters most.</h1><p className="hero-lede">CivicPulse turns scattered citizen reports into clear, impact-driven action for the people who keep cities moving.</p><div className="hero-buttons"><button className="primary-button">See CivicPulse in action <ArrowRight data-icon="inline-end" /></button><button className="text-button">Explore the platform <ArrowRight data-icon="inline-end" /></button></div><div className="trust-row"><div className="trust-avatars"><span>RK</span><span>PM</span><span>AS</span><span>+</span></div><p>Trusted by teams building<br /><strong>more responsive cities</strong></p></div></div><div className="hero-visual"><div className="visual-label"><span className="status-dot" /> AUTHORITY VIEW <span>•</span> LIVE</div><DashboardMockup /></div></section>
